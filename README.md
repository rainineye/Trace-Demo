...
1. User visits any path on the site
2. `middleware.js` checks for the `trace_auth` cookie
3. If missing or invalid → redirect to `/login?from=<original path>`
4. Login page POSTs password to `/api/auth`
5. If password matches `AUTH_PASSWORD` → set HTTP-only cookie with `AUTH_TOKEN`, valid 7 days
6. Redirect back to `<original path>`
7. Subsequent requests pass middleware silently until cookie expires
...
3. In **Environment Variables**, add:
   - `AUTH_PASSWORD` (same value as local)
   - `AUTH_TOKEN` (same value as local)
4. Deploy
5. Go to **Settings → Domains** → add `demo.0xmian.com`

### Subsequent deploys

Every `git push` to `main` triggers an automatic rebuild and redeploy on Vercel. No manual action needed.

### Changing the password

1. Vercel dashboard → project → **Settings → Environment Variables**
2. Edit `AUTH_PASSWORD`
3. Go to **Deployments** tab → latest deployment → `⋯` → **Redeploy**

Changing `AUTH_PASSWORD` alone does *not* invalidate existing sessions (they still have valid cookies). To also force all users to re-login, change `AUTH_TOKEN` at the same time.
